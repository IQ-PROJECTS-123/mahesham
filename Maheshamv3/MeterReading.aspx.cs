...
namespace Maheshamv3
{
    public partial class MeterReading : Page
    {
        ...
        protected void _ButtonSubmit_Click(object sender, EventArgs e)
        {
            ...
            Utility.ExecuteQuery(@"IF EXISTS (SELECT 1 FROM MeterReading WHERE Facility=@Facility AND Year=@Year AND MonthNo=@MonthNo)UPDATE MeterReading SET Reading=@Reading,ReadingFile=@ReadingFile,ReadingOn=@ReadingOn,ReadingBy=@ReadingBy WHERE Facility=@Facility AND Year=@Year AND MonthNo=@MonthNo
                ELSE
            ...
            foreach (DataRow dr in tenants.Rows)
            {
                ...
                else
                {
                    Utility.ExecuteQuery(@"INSERT INTO Rent(Facility, Tenant,eUnitCost, Amount, PeriodStart, PeriodEnd,MeterStart, MeterEnd, rMonth, rYear, rMonthNo,Maintenance, TotalAmount, Active, Due, AmountType)
                        VALUES(@Facility, @Tenant, @Amount,@eUnitCost, @PeriodStart, @PeriodEnd,@MeterStart, @MeterEnd, @rMonth, @rYear, @rMonthNo,@Maintenance, @TotalAmount, 1, @TotalAmount, 'Rental')",
                        false,
