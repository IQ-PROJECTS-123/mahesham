...
<<<<<<< HEAD
            ...
            // ===== CASH / SCAN / BANK SPLIT LOGIC =====
            decimal cashAmount = 0;
            decimal scanAmount = 0;
            decimal bankAmount = 0;
            string paymentType = _DropDownListType.SelectedValue;

            if (paymentType == "Cash")
            {
                cashAmount = paidAmount;
            }
            else if (paymentType == "Scan")
            {
                scanAmount = paidAmount;
            }
            ...
            {
                cashAmount = Convert.ToDecimal(_txtCashAmount.Text);
                scanAmount = Convert.ToDecimal(_txtUpiAmount.Text);

                if (cashAmount + scanAmount != paidAmount)
                {
                    _LiteralMSG.Text = "<div class='p-3 mb-2 bg-danger text-white'>Cash + UPI ka total galat hai</div>";
                    return;
                }
            }
            Utility.ExecuteQuery(@"
                UPDATE Rent SET PaidAmount=@PaidAmount,PaymentFile=@PaymentFile,Due=@Due,PaidOn=@PaidOn,Status='Completed',PaymentType=@PaymentType,Remarks=@Remarks,CashAmount=@CashAmount,ScanAmount=@ScanAmount,BankAmount=@BankAmount WHERE ID=@ID",
                false,
