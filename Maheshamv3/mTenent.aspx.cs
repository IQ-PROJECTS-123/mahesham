    ...
    public partial class mTenent : System.Web.UI.Page
    {
        ...
        private void LoadTenantData(string tenantId)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                // Resolved: Include Electric Unit Cost binding
        ...
        protected void _ButtonSubmit_Click(object sender, EventArgs e)
        {
            ...
            // Resolved: Validations from HEAD branch
            ...
            // Resolved: SQL Query includes 'eUnitCost'
            string query = string.IsNullOrEmpty(tenantId)
                ? @"INSERT INTO Tenant(MeterReadingStart,eUnitCost,TenantType,Name,Mobile1,Mobile2,Email,PWD,Address,FatherName,HomeNumber,AadharNumber,PANNumber,VoterNumber,Facility,MonthlyRent,Maintenance,Advance,RentStart,Active) 
                    VALUES(@MeterReadingStart,@eUnitCost,@TenantType,@Name,@Mobile1,@Mobile2,@Email,@PWD,@Address,@FatherName,@HomeNumber,@AadharNumber,@PANNumber,@VoterNumber,@Facility,@MonthlyRent,@Maintenance,@Advance,@RentStart,1)"
            ...
            Utility.ExecuteQuery(query, false,
                ...
                new SqlParameter("@eUnitCost", Convert.ToInt32(_DropDownMeter.SelectedValue)), // Resolved: Added eUnitCost param
        ...
        protected void _DropDownListFacility_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                // Resolved: Select eUnitCost to populate partner data correctly
        ...
        protected void BindRooms()
        {
            string query = _DropDownListType.SelectedValue == "Main Tenant"
                ? @"SELECT ID, Building+' '+Location+' - '+Title AS Title FROM Facility WHERE NOT ID IN (SELECT Facility FROM Tenant WHERE Active=1 AND TenantType='Main Tenant') ORDER BY ID"
                : @"SELECT DISTINCT f.ID, f.Building+' '+f.Location+' - '+f.Title AS Title FROM Facility f INNER JOIN Tenant t ON t.Facility=f.ID WHERE t.Active=1 AND t.TenantType='Main Tenant' ORDER BY f.ID";
